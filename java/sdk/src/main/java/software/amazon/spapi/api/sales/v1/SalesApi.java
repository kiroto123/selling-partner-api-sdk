...
public class SalesApi {
     ...
     *     Only one marketplace can be specified. For example, ATVPDKIKX0DER indicates the US marketplace. (required)
     * @param interval A time interval used for selecting order metrics. This takes the form of two dates separated by
     ...
     *     -07:00 zone. (required)
     * @param granularity The granularity of the grouping of order metrics, based on a unit of time. Specifying
     *     granularity&#x3D;Hour results in a successful request only if the interval specified is less than or equal to
     *     30 days from now. For all other granularities, the interval specified must be less or equal to 2 years from
     *     now. Specifying granularity&#x3D;Total results in order metrics that are aggregated over the entire interval
     *     that you specify. If the interval start and end date don’t align with the specified granularity, the head and
     *     tail end of the response interval will contain partial data. Example: Day to get a daily breakdown of the
     *     request interval, where the day boundary is defined by the granularityTimeZone. (required)
     * @param granularityTimeZone An IANA-compatible time zone for determining the day boundary. Required when
     *     specifying a granularity value greater than Hour. The granularityTimeZone value must align with the offset of
     *     the specified interval value. For example, if the interval value uses Z notation, then granularityTimeZone
     *     must be UTC. If the interval value uses an offset, then granularityTimeZone must be an IANA-compatible time
     *     zone that matches the offset. Example: US/Pacific to compute day boundaries, accounting for daylight time
     *     savings, for US/Pacific zone. (optional)
     * @param buyerType Filters the results by the buyer type that you specify, B2B (business to business) or B2C
     *     (business to customer). Example: B2B, if you want the response to include order metrics for only B2B buyers.
     *     (optional, default to All)
     * @param fulfillmentNetwork Filters the results by the fulfillment network that you specify, MFN (merchant
     *     fulfillment network) or AFN (Amazon fulfillment network). Do not include this filter if you want the response
     *     to include order metrics for all fulfillment networks. Example: AFN, if you want the response to include
     *     order metrics for only Amazon fulfillment network. (optional)
     * @param firstDayOfWeek Specifies the day that the week starts on when granularity&#x3D;Week, either Monday or
     *     Sunday. Default: Monday. Example: Sunday, if you want the week to start on a Sunday. (optional, default to
     *     Monday)
     * @param asin Filters the results by the ASIN that you specify. Specifying both ASIN and SKU returns an error. Do
     *     not include this filter if you want the response to include order metrics for all ASINs. Example: B0792R1RSN,
     *     if you want the response to include order metrics for only ASIN B0792R1RSN. (optional)
    ...
    private okhttp3.Call getOrderMetricsCall(
        ...
        if (interval != null) localVarQueryParams.addAll(apiClient.parameterToPair("interval", interval));
        if (granularityTimeZone != null)
            localVarQueryParams.addAll(apiClient.parameterToPair("granularityTimeZone", granularityTimeZone));
        if (granularity != null) localVarQueryParams.addAll(apiClient.parameterToPair("granularity", granularity));
        if (buyerType != null) localVarQueryParams.addAll(apiClient.parameterToPair("buyerType", buyerType));
    ...
    private okhttp3.Call getOrderMetricsValidateBeforeCall(
            List<String> marketplaceIds,
            String interval,
            String granularity,
            String granularityTimeZone,
        ...
        if (marketplaceIds == null) {
            throw new ApiException(
                    "Missing the required parameter 'marketplaceIds' when calling getOrderMetrics(Async)");
        }
    ...
    /**
     * Returns aggregated order metrics for given interval, broken down by granularity, for given buyer type. **Usage
     * Plan:** | Rate (requests per second) | Burst | | ---- | ---- | | .5 | 15 | The &#x60;x-amzn-RateLimit-Limit&#x60;
     ...
     * @throws LWAException If calls to fetch LWA access token fails
     */
    public GetOrderMetricsResponse getOrderMetrics(
            List<String> marketplaceIds,
            String interval,
            String granularity,
            String granularityTimeZone,
     ...
     * @throws LWAException If calls to fetch LWA access token fails
     */
    public ApiResponse<GetOrderMetricsResponse> getOrderMetricsWithHttpInfo(
            List<String> marketplaceIds,
            String interval,
            String granularity,
            String granularityTimeZone,
    ...
    public ApiResponse<GetOrderMetricsResponse> getOrderMetricsWithHttpInfo(
            ...
            String amazonProgram)
            throws ApiException, LWAException {
        return getOrderMetricsWithHttpInfo(
                marketplaceIds,
                interval,
                granularity,
                granularityTimeZone,
    ...
    /**
     * (asynchronously) Returns aggregated order metrics for given interval, broken down by granularity, for given buyer
     * type. **Usage Plan:** | Rate (requests per second) | Burst | | ---- | ---- | | .5 | 15 | The
     ...
     * @throws LWAException If calls to fetch LWA access token fails
     */
    public okhttp3.Call getOrderMetricsAsync(
            List<String> marketplaceIds,
            String interval,
            String granularity,
            String granularityTimeZone,
